...
{
    window.location.href = "/sinavsablonekle";
}
...
async function sinavSablonListeYukle()
{
    try
    {
        const response = await fetch('sinavsablonliste/oku/');
...
async function sinavSablonSil()
{
    ...
    try
    {
        const response = await fetch('/sinavsablonliste/sil/' + encodeURIComponent(sablonId),
        {
            method: "DELETE"
        });
        const sonuc = await response.json().catch(function () { return {}; });
...
function sinavSablonDuzenleGoster(button)
{
    ...
    window.location.href = "/sinavsablonduzenle?id=" + encodeURIComponent(sablonId);
}
