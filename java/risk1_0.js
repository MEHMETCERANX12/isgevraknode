const RISK_STORE =
{
    duzenleVeri: "riskduzenleveri",
    duzenleKayit: "riskduzenlekayitjson",
    duzenleYeniAd: "riskduzenleyeniad",
    liste: "riskdegerlendirmeliste",
    silinecekId: "risksilinecekid",
    ciktiGenelListe: "riskciktigenelliste",
    ciktiSecilenIds: "riskciktisecilenids",
    ciktiSeciliListe: "riskciktiseciliriskler",
    ciktiCalisanListe: "riskcikticalisanliste"
};
...
{
    var data = riskStoreJsonOku(RISK_STORE.liste);
    if (data)
    {
        try
        {
    ...
    $(document).off('click.riskduzenle1', 'input[name="duzenle"], input[name="sil"]').on('click.riskduzenle1', 'input[name="duzenle"], input[name="sil"]', function ()
    {
        ...
        {
            store.set(RISK_STORE.silinecekId, i);
...
function risksilr6()
{
    const iddeger = store.get(RISK_STORE.silinecekId);
...
async function riskduzenle2load()
{
    ...
    try
    {
        ...
        store.set(RISK_STORE.duzenleVeri, json);
        duzenleload2r2();
...
{
    const riskid = riskduzenle2idoku();
    const json = riskStoreJsonOku(RISK_STORE.duzenleKayit);
...
{
    const riskid = riskduzenle2idoku();
    const ad = String(store.get(RISK_STORE.duzenleYeniAd) || "").trim();
    const json = riskStoreJsonOku(RISK_STORE.duzenleKayit);
    if (!riskid)
    {
        alertify.error("Beklenmedik bir hata oluştu");
...
{
    let calisanjson = riskStoreJsonOku(RISK_STORE.ciktiCalisanListe);
    ...
    let riskjson = riskStoreJsonOku(RISK_STORE.ciktiSeciliListe);
...
async function riskcikti3load()
{
    try
    {
        ...
        store.set(RISK_STORE.ciktiSeciliListe, Array.isArray(json.riskjson) ? json.riskjson : []);
        store.set(RISK_STORE.ciktiCalisanListe, Array.isArray(json.calisanjson) ? json.calisanjson : []);
        return true;
    }
    catch (error)
    {
...
async function riskcikti2load()
{
    try
    {
        ...
        store.set(RISK_STORE.ciktiGenelListe, Array.isArray(json) ? json : []);
...
{    
    let json = riskStoreJsonOku(RISK_STORE.ciktiGenelListe);
...
function ciktidevamr5()
{
    ...
    {
        store.set(RISK_STORE.ciktiSecilenIds, liste);
        return true;
    }
}
...
function ciktidevamr5node()
{
    ...
    const ids = riskStoreJsonOku(RISK_STORE.ciktiSecilenIds);
...
{    
    ...
    veri.x[0] = yenibaslik;
    riskDuzenleVeriYaz(veri);
    store.set(RISK_STORE.duzenleYeniAd, yenibaslik);
